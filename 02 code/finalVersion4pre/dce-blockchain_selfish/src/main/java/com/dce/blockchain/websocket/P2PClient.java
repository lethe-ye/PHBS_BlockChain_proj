...
public class P2PClient {
	 ...
	 * 连接自私节点
	 */
	public void connectToSelfishPeer(String addr) {
		try {
			final WebSocketClient socketClient = new WebSocketClient(new URI(addr)) {
				@Override
				public void onOpen(ServerHandshake serverHandshake) {
					//客户端发送请求，查询最新区块
					p2pService.write(this, p2pService.queryLatestBlockMsg());
					p2pService.getSelfishSockets().add(this);
				}
				...
				@Override
				public void onMessage(String msg) {
					p2pService.handleMessage(this, msg, p2pService.getSelfishSockets());
				}

				@Override
				public void onClose(int i, String msg, boolean b) {
					p2pService.getSelfishSockets().remove(this);
					System.out.println("connection closed");
				}
