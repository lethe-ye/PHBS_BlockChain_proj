...
public class BlockService {
	...
	public String createGenesisBlock() throws UnsupportedEncodingException {
		KeyPair keyPairGenesis;
		keyPairGenesis = keyPairGen.generateKeyPair();
		System.out.println("keyPairGenesis");
		//TODO: 把下面的换成coinBaseTxs之类的
		List<Transaction> tsaList = new ArrayList<Transaction>();
		Transaction tsa1 = new Transaction(COINBASE,keyPairGenesis.getPublic());
		System.out.println("tsa1");

		tsa1.setId("1");
		tsa1.setBusinessInfo("这是创世区块,Miner Address = "+keyPairGenesis.getPublic());
		tsaList.add(tsa1);
		...
		byte[] genesisPrevHash = ("genesisBlock").getBytes();
		byte[] genesisBlockHash = calculateHash(genesisPrevHash,tsaList,1);
	 ...
	 * @return
	 */
	public Block createNewBlock(int nonce, byte[] previousHash, List<Transaction> blockTxs, byte[] thisBlockHash) {
...
//		blackList.add("someone's publicKey");
		...
		if (!previousBlock.getHash().equals(newBlock.getPreviousHash())) {
			...
			// (!hash.equals(newBlock.getHash()))
			if (!Arrays.equals(hash,newBlock.getHash())) {
	...
	public boolean isValidHash(byte[] hash) {
		String hashStr = new String(hash,0,20);
	 ...
	 * @return
	 */
	public byte[] calculateHash(byte[] previousHash, List<Transaction> currentTransactions, int nonce) throws UnsupportedEncodingException {
		String prevHashStr = CryptoUtil.byte2Hex(previousHash);
		String thisHashStr = CryptoUtil.SHA256(prevHashStr + JSON.toJSONString(currentTransactions) + nonce);
		return thisHashStr.getBytes();
	}
