	...
	public void handleSelfMessage(String msg) {
		try {
			...
			switch (message.getType()) {
				//客户端请求查询最新的区块:1
				case BlockConstant.QUERY_LATEST_BLOCK:
					System.out.println("BlockConstant.QUERY_LATEST_BLOCK");
					System.out.println(responseLatestBlockMsg());//服务端调用方法返回最新区块:2
					break;
				//接收到服务端返回的最新区块:2
				case BlockConstant.RESPONSE_LATEST_BLOCK:
					System.out.println("BlockConstant.RESPONSE_LATEST_BLOCK");
					handleBlockResponse(message.getData());
					break;
				//客户端请求查询整个区块链:3
				case BlockConstant.QUERY_BLOCKCHAIN:
					System.out.println("BlockConstant.QUERY_BLOCKCHAIN");
					System.out.println(responseBlockChainMsg());//服务端调用方法返回最新区块:4
					break;
				//直接接收到其他节点发送的整条区块链信息:4
				case BlockConstant.RESPONSE_BLOCKCHAIN:
					System.out.println("BlockConstant.RESPONSE_BLOCKCHAIN");
					handleBlockChainResponse(message.getData());
					break;
			}
		} catch (Exception e) {
...
//    	public synchronized void handleBlockResponse(String blockData, List<WebSocket> sockets) {
		...
		if (latestBlockReceived != null) {
			System.out.println("latestBlockReceived != null");
			if(latestBlock != null) {
				...
				if(latestBlockReceived.getIndex() > latestBlock.getIndex() + 1) {
					...
					// 还是broadcast blockChain比较好，这样肯定可以覆盖掉
					System.out.println("应该指broadcast 最近的两个block");
					broatcast(responsePartialBlockChainMsg(latestBlockReceived.getIndex()+1));
//					broatcast(responseBlockMsg(latestBlockReceived.getIndex()));
//					broatcast(responseBlockMsg(latestBlockReceived.getIndex()+1));
					System.out.println("另一些node应该会有重新更新blockChain");


				}
				else{System.out.println(latestBlock.getHash().equals(latestBlockReceived.getPreviousHash()));}
			}else if(latestBlock == null) {
				broatcast(queryBlockChainMsg());
				System.out.println("重新查询所有节点上的整条区块链");
			}
		}
		else{System.out.println("latestBlockReceived != null");}
	 ...
	 * @return
	 */
	public String responseBlockMsg(int i) {
		Message msg = new Message();
		msg.setType(BlockConstant.RESPONSE_LATEST_BLOCK);
		Block b = blockCache.getBlock(i);
		msg.setData(JSON.toJSONString(b));
		return JSON.toJSONString(msg);
	}
